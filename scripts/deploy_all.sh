...
gcloud run deploy youtube-analyst-backend \
    ...
    --min-instances 0 \
    --max-instances 10 \
    --set-env-vars "SANDBOX_SERVICE_URL=${SANDBOX_URL},GCP_PROJECT_ID=${PROJECT_ID},GCP_REGION=${REGION},VERTEX_AI_REGION=us,GEMINI_MODEL=gemini-3.8-flash,YOUTUBE_API_KEY=${YOUTUBE_API_KEY},TELEGRAM_BOT_TOKEN=${TELEGRAM_BOT_TOKEN},TELEGRAM_WEBHOOK_SECRET=${TELEGRAM_WEBHOOK_SECRET},BIGQUERY_DATASET_ID=${BIGQUERY_DATASET_ID:-youtube_analytics},GCS_BUCKET_NAME=${GCS_BUCKET_NAME:-gen-lang-client-0428255657-yt-raw-data},CLOUD_TASKS_QUEUE=${CLOUD_TASKS_QUEUE:-telegram-tasks},BACKEND_PUBLIC_URL=${BACKEND_PUBLIC_URL},TELEGRAM_ADMIN_CHAT_ID=${TELEGRAM_ADMIN_CHAT_ID:-},TELEGRAM_ALLOWED_USERS=${TELEGRAM_ALLOWED_USERS:-}" \
    --project "${PROJECT_ID}" \
    --allow-unauthenticated
